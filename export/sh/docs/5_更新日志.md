...
# Docker Export Compose v2.2.2 更新说明
...
### 1. 安静模式（--quiet）导致脚本提前退出

**问题描述**：
- 使用 `--quiet` 模式时，脚本会提前退出
- 导致文件完全没有创建
- 没有任何错误提示

**影响范围**：
- ❌ 所有使用 `--quiet` 模式的场景 100% 失效
- ❌ 组合使用 `--quiet` 和其他选项全部失败

**修复内容**：
- ✅ 修复 log_info()、log_warn()、log_success() 函数
- ✅ 修复 show_summary() 函数，支持安静模式
- ✅ 修复容器列表显示、横幅显示等代码块
- ✅ 共修复 7 处相关代码

---

### 2. 隐私模式（--privacy）注释泄露原始路径

**问题描述**：
- 使用 `--privacy` 模式时，虽然路径被隐藏
- 但注释中仍显示了完整的原始主机路径
- 违反了隐私保护的初衷
...
```yaml
volumes:
  - /path/to/data:/var/lib/mysql  # 原路径已隐藏 / Original: /home/user/mysql-data
  #                                                  ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  #                                                  实际路径暴露了！
```
...
```yaml
volumes:
  - /path/to/data:/var/lib/mysql  # 原路径已隐藏以保护隐私 / Original path masked for privacy
  #                                  路径完全隐藏
```
...
# 3. 测试安静模式
./docker-export-compose.sh --quiet my-container
ls -la ./output/my-container/  # 验证文件已创建

# 4. 测试隐私模式（如果之前使用过）
./docker-export-compose.sh --privacy my-container
cat ./output/my-container/docker-compose.yml | grep volumes
# 验证注释中不再显示原始路径
```
...
```bash
# --quiet 模式问题：不要使用 --quiet
./docker-export-compose.sh my-container  # 使用正常模式
./docker-export-compose.sh my-container 2>/dev/null  # 或重定向输出

# --privacy 模式问题：导出后手动编辑配置文件
./docker-export-compose.sh --privacy my-container
# 然后手动检查并删除注释中的原始路径信息
```
